...
namespace Update
{
    public partial class FormUpdate : Form
    {
        ...
        public FormUpdate()
        {
            ...
            url = "http://apitest.radaredu.cn/EduApi/download/";// service.GetUrl();//获取下载地址
            //zips = service.GetZips();//获取升级压缩包
            zips = new string[2]; 
            zips[1] = "php.zip";
            zips[0] = "edu_update_test.zip";
        }
        ...
        private void DownLoad()
        {
            try
            {
                ...
                {
                    wc = new WebClient();
                    string urlpath = url + zips[zipsIndex];
                    wc.DownloadProgressChanged += new DownloadProgressChangedEventHandler(wc_DownloadProgressChanged);
                    wc.DownloadFileCompleted += new AsyncCompletedEventHandler(wc_DownloadFileCompleted);
                    wc.DownloadFileAsync(new Uri(urlpath), zips[zipsIndex]);
                }
                else
                {
