...
namespace RueHelper
{
    ...
    {        
        ...
        public static void init(string _PhoneNumber, string _AppKey,string _SecretKey)
        {
        ...
        public static int HandlerWork(IntPtr SessionId, uint MsgType, uint Error, string Info, IntPtr pUser)
        {
            switch (MsgType)
            {
                case 20:
                    ...
                    if (Error == 0)
                    {
                        FormCamera.jObjInfo = obj;
                        FormCamera.isOpertion = 2;
                    }
                    break;
                case 3:// 播放开始
                    break;
                ...
                case 5:// 播放结束，回放结束时会有此消息
                    FormCamera.mType = FormCamera.MessageType.INS_PLAY_ARCHIVE_END;
                    break;
                default:
                    break;
            }
